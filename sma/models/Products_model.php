...
//     public function getStimaItemByID($id)
//    {
//        // $this->load->database('default1', TRUE);
//        // $this->db->get_where('quote_items', array('id' => $id), 1);
//         $q2 = $this->db2->get_where('menus', array('menu_id' => $id), 1);
//        if ($q2->num_rows() > 0) {
//            return $q2->row();
//        }
//        return FALSE;
...
//        // $this->load->database('default1', TRUE);
//        // $this->db->get_where('quote_items', array('id' => $id), 1);
//         $q2 = $this->db2->get_where('categories', array('category_id' => $id), 1);
//        if ($q2->num_rows() > 0) {
//            return $q2->row();
//        }
//        return FALSE;
//    }
     public function getProductsByWarehouseID($whid)
    {
//                  // $q = $this->db->get('products');
//              $q = $this->db->select($this->db->dbprefix('products') . ".* ")
//                ->from('products')
//                ->join('warehouses_products', 'products.id=warehouses_products.product_id', 'left')
//                ->where('warehouses_products.warehouse_id', $whid)
...
//        return FALSE;
         ...
         $this->db->select('sma_products.*,sma_warehouses_products.product_id')
            ->join('sma_warehouses_products', 'sma_warehouses_products.product_id=products.id', 'left')
            ->where('warehouses_products.warehouse_id', $whid)
            ->order_by('sma_products.id');
        //$q = $this->db->get_where('sma_warehouses_products', array('sma_warehouses_products.warehouse_id' => 1), 1);
    ...
    public function getPortionNames($term, $limit)
    {
        $this->db->select("id, name as text", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('products', array('iskitchen' => 0), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getPurchasedItemDetails($product_id, $warehouse_id)
    {
	    ...
	    public function getPurchasedItemDetailsEdit($product_id, $warehouse_id)
    {
        $q = $this->db->get_where('purchase_items', array('product_id' => $product_id, 'warehouse_id' => $warehouse_id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
...
// public function getStimaCategoryById($id)
//    {
//        $q = $this->db2->get_where('categories', array('category_id' => $id), 1);
//        if ($q->num_rows() > 0) {
//            return $q->row();
//        }
//
...
//    }
    ...
    public function syncAdjustment($data = array())
    {
        if(! empty($data)) {
            if($data['type'] == 'subtraction') {
                if ($purchase_item = $this->getPurchasedItemDetailsEdit($data['product_id'], $data['warehouse_id'])) {
  ...
  public function syncAdjustment2($data = array())
    {
        if(! empty($data)) {
            if($data['type'] == 'subtraction') {
    ...
    public function addQuantity($product_id, $warehouse_id, $quantity, $rack = NULL)
    {
        ...
        if ($this->products_model->getProductQuantity($product_id, $warehouse_id)) {
...
//  $db2=$this->load->database('default1', TRUE);
//              $q2 = $db2->get_where('products', array('menu_id' =>($this->input->post('name'))), 1);
