...
class Reports_model extends CI_Model
{
    ...
    public function getCustomerSales($customer_id)
    {
      ...
      public function getCustomerDueSales($customer_id)
    {
        $this->db->where('customer_id', $customer_id)->where('payment_status',"due");
        $q = $this->db->get('sales');
       if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    {
        $q = $this->db->query("SELECT SUM(by_price) as stock_by_price, SUM(by_cost) as stock_by_cost 
            FROM ( Select sum(COALESCE(" . $this->db->dbprefix('warehouses_products') . ".quantity, 0))*price 
                as by_price, sum(COALESCE(" . $this->db->dbprefix('warehouses_products') . ".quantity, 0))*cost as by_cost 
                    FROM " . $this->db->dbprefix('products') . " JOIN " . $this->db->dbprefix('warehouses_products') .
                " ON " . $this->db->dbprefix('warehouses_products') . ".product_id=" . $this->db->dbprefix('products') . 
                ".id WHERE " . $this->db->dbprefix('warehouses_products') . ".warehouse_id = ? GROUP BY " . 
   ...
   $myQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( total_discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
    ...
    public function getMonthlySales($year)
    {
		...
		//	GROUP BY date_format( date, '%c' ) ORDER BY date_format( date, '%c' ) ASC";
			...
			$myQuery = "SELECT DATE_FORMAT(date,  '%c' ) AS date,SUM( COALESCE( item_tax, 0 ) ) AS tax1,SUM( COALESCE( order_tax, 0 ) ) AS tax2,SUM( COALESCE( subtotal, 0 ) ) AS total,SUM( COALESCE( item_discount, 0 ) ) AS discount,0 AS shipping
					FROM " . $this->db->dbprefix('sales') . "," . $this->db->dbprefix('sale_items') . "
					WHERE ".$this->db->dbprefix('sales').".id = ".$this->db->dbprefix('sale_items').".sale_id  AND DATE_FORMAT( date,  '%Y' ) =  '{$year}' 
					GROUP BY date_format( date, '%c' ) ORDER BY date_format( date, '%c' ) ASC";
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
       ...
       //     FROM " . $this->db->dbprefix('sales') . "
        //    WHERE created_by = {$user_id} AND DATE_FORMAT( date,  '%Y-%m' ) =  '{$year}-{$month}'
         //   GROUP BY DATE_FORMAT( date,  '%e' )";
			...
			$myQuery = "SELECT DATE_FORMAT( date,  '%e' ) AS date,SUM( COALESCE( item_tax, 0 ) ) AS tax1,SUM( COALESCE( order_tax, 0 ) ) AS tax2,SUM( COALESCE( subtotal, 0 ) ) AS total,SUM( COALESCE( item_discount, 0 ) ) AS discount,0 AS shipping
					FROM " . $this->db->dbprefix('sales') . "," . $this->db->dbprefix('sale_items') . "
					WHERE ".$this->db->dbprefix('sales').".id = ".$this->db->dbprefix('sale_items').".sale_id AND created_by = {$user_id} AND DATE_FORMAT( date,  '%Y-%m' ) =  '{$year}-{$month}'
					GROUP BY DATE_FORMAT( date,  '%e' )";
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getStaffMonthlySales($user_id, $year)
    {
       $myQuery = "SELECT DATE_FORMAT(date,  '%c' ) AS date,SUM( COALESCE( item_tax, 0 ) ) AS tax1,SUM( COALESCE( order_tax, 0 ) ) AS tax2,SUM( COALESCE( subtotal, 0 ) ) AS total,SUM( COALESCE( item_discount, 0 ) ) AS discount,0 AS shipping
					FROM " . $this->db->dbprefix('sales') . "," . $this->db->dbprefix('sale_items') . "
					WHERE ".$this->db->dbprefix('sales').".id = ".$this->db->dbprefix('sale_items').".sale_id AND created_by = {$user_id} AND DATE_FORMAT( date,  '%Y' ) =  '{$year}' 
					GROUP BY date_format( date, '%c' ) ORDER BY date_format( date, '%c' ) ASC";
       // $myQuery = "SELECT DATE_FORMAT( date,  '%c' ) AS date, SUM( COALESCE( product_tax, 0 ) ) AS tax1, SUM( COALESCE( order_tax, 0 ) ) AS tax2, SUM( COALESCE( grand_total, 0 ) ) AS total, SUM( COALESCE( total_discount, 0 ) ) AS discount, SUM( COALESCE( shipping, 0 ) ) AS shipping
       //     FROM " . $this->db->dbprefix('sales') . "
