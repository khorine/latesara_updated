...
//    }
	...
	public function getTables($id)
    {
        ...
        $this->db->select('table.id,name,reference_no,payment_status')
		        ->join('sales', 'table.id=sales.table_id AND sma_sales.created_by ='.$id.' AND payment_status <> "paid" AND pos = "1" ', 'left')
            ->group_by('table.name')
			->order_by('id');
       $q = $this->db->get('table');
              if ($q->num_rows() > 0) {
...
elseif($warehouse=='32'){
		$q =$this->db->query("SELECT * from (SELECT code,name,SUM(saleqty) as saleqty,SUM(purchaseqty) AS purchaseqty,SUM(adjstqty) AS adjstqty,SUM(saleopqty) as saleopqty,SUM(purchaseopqty) as purchaseopqty,SUM(adjstopqty) as adjstopqty,id,unit FROM (
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_products WHERE sma_products.category_id < '57' AND sma_products.category_id <> '53'  UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_sale_items` ON `sma_products`.`id` = `sma_sale_items`.`product_id`
...
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
...
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,SUM(IF(sma_adjustments.type='subtraction',sma_adjustments.quantity*-1,sma_adjustments.quantity)) as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_adjustments` ON `sma_products`.`id` = `sma_adjustments`.`product_id`
...
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty,0 as purchaseqty,0 as adjstqty, SUM(COALESCE(sma_sale_items.quantity, 0 )) as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_sale_items` ON `sma_products`.`id` = `sma_sale_items`.`product_id`
...
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as adjstqty,0 as saleopqty,SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
...
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,SUM(IF(sma_adjustments.type='subtraction',sma_adjustments.quantity*-1,sma_adjustments.quantity)) as adjstopqty FROM sma_products 
LEFT JOIN `sma_adjustments` ON `sma_products`.`id` = `sma_adjustments`.`product_id`
...
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`portion1` as id,unit,sma_products.portion1 as code, sma_products.name ,SUM(COALESCE(sma_products.portion1qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion1`   UNION
SELECT `sma_products`.`portion2`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion2qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion2`  UNION 
SELECT `sma_products`.`portion3`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion3qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion3`  UNION 
SELECT `sma_products`.`portion4`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion4qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion4`  UNION
SELECT `sma_products`.`portion5`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion5qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion5`  UNION 
SELECT `sma_products`.`portion6`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion6qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion6`  UNION
SELECT `sma_products`.`portion7`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion7qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion7`) T6 GROUP BY `id`)T8 WHERE id = '$pro'  ");
}
              if ($q->num_rows() > 0) {
            return $q->row();
        }
	...
	public function getPOSCategories() {
        ...
        $q = $this->db->get_where('categories',array('inv' => '0'));
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
...
public function getAllCategories() {
    ...
    public function calculateAVCost($product_id, $warehouse_id, $net_unit_price, $unit_price, $quantity, $iskitchen, $product_name, $option_id, $item_quantity) {
    ...
    public function item_costing($item, $pi = NULL) {
        ...
        if ($this->Settings->accounting_method != 2 && !$this->Settings->overselling) {
            ...
            if ($this->site->getProductByID($item['product_id'])) {
                if ($item['product_type'] == 'standard') {
                    $cost = $this->site->calculateAVCost($item['product_id'], $item['warehouse_id'], $item['net_unit_price'], $item['unit_price'], $item['quantity'],$item['iskitchen'], $item['product_name'], $item['option_id'], $item_quantity);
                } elseif ($item['product_type'] == 'combo') {
                    $combo_items = $this->getProductComboItems($item['product_id'], $item['warehouse_id']);
                    foreach ($combo_items as $combo_item) {
                        $cost = $this->site->calculateAVCost($combo_item->id, $item['warehouse_id'], ($combo_item->qty * $item['quantity']), $item['unit_price'], $item['quantity'], $item['iskitchen'], $item['product_name'], $item['option_id'], $item_quantity);
                    }
                } else {
                    $cost = array(array('date' => date('Y-m-d'), 'product_id' => $item['product_id'], 'sale_item_id' => 'sale_items.id', 'purchase_item_id' => NULL, 'quantity' => $item['quantity'], 'purchase_net_unit_cost' => 0, 'purchase_unit_cost' => 0, 'sale_net_unit_price' => $item['net_unit_price'], 'sale_unit_price' => $item['unit_price'], 'quantity_balance' => NULL, 'inventory' => NULL));
                }
