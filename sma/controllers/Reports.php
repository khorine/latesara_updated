...
class Reports extends MY_Controller
{
    ...
    function warehouse_stock($warehouse = NULL)
    {
	    ...
	    function stock_sheet($warehouse = NULL)
    {
                $this->sma->checkPermissions();
        ...
        $meta = array('page_title' => lang('Stock_Sheet'), 'bc' => $bc);
        $this->page_construct('reports/stocksheet', $meta, $this->data);
	...
	function getStockSheet($pdf = NULL, $xls = NULL)
    {
        ...
        if ($start_date) {
            ...
            $pp = "( SELECT pi.product_id, SUM( pi.quantity ) purchasedQty, SUM( pi.subtotal ) totalPurchase from " . $this->db->dbprefix('purchase_items') . " pi JOIN " . $this->db->dbprefix('purchases') . " p on p.id = pi.purchase_id where p.date >= '{$sdate}' and p.date < '{$tdate}' group by pi.product_id ) PCosts";
            $sp = "( SELECT si.product_id, SUM( si.quantity ) soldQty, SUM( si.subtotal ) totalSale from " . $this->db->dbprefix('sale_items') . " si JOIN " . $this->db->dbprefix('sales') . " s on s.id = si.sale_id where s.date >= '{$sdate}' and s.date < '{$tdate}'  group by si.product_id ) PSales";
        }
        if ($pdf || $xls) {
            ...
            $this->db
               ->select($this->db->dbprefix('products') . ".code, " . $this->db->dbprefix('products') . ".name,
				...
				COALESCE(".$this->db->dbprefix('products') . ".cost * SoldQty,0) as pcost,
				COALESCE( PSales.totalSale, 0 ) as TotalSales,
				(COALESCE( PSales.totalSale, 0 ) - COALESCE(".$this->db->dbprefix('products') . ".cost * SoldQty,0)) as Profit", FALSE)
                ->from('products')
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('E1', lang('sold'));
                $this->excel->getActiveSheet()->SetCellValue('F1', lang('sold cost'));
				...
				$pcost = 0;
                foreach ($data as $data_row) {
                    $profit =  $data_row->TotalSales - $data_row->pcost;
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $data_row->code);
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->name);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->SoldQty);
                     $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->pcost);
					...
					$pcost += $data_row->pcost;
                    $pl += $profit;
                    $row++;
                }
                $this->excel->getActiveSheet()->getStyle("C" . $row . ":H" . $row)->getBorders()
				...
				$this->excel->getActiveSheet()->SetCellValue('F' . $row, $pcost );
			...
			->select($this->db->dbprefix('products') . ".code, " . $this->db->dbprefix('products') . ".name,'0' AS OpStock,".$this->db->dbprefix('products').".unit as Uom,
			SUM(COALESCE(".$this->db->dbprefix('sale_items'). ".quantity, 0 )) as saleqty,
			".$this->db->dbprefix('products'). ".price as rate,
			sma_products.price  * SUM(COALESCE( sma_sale_items.quantity, 0 )) as amount,
			SUM(sma_adjustments.quantity) AS adjst" , FALSE)
			->from('products')
			 ->join('sma_sale_items', 'sma_products.id = sma_sale_items.product_id', 'left')
			 ->join('sma_adjustments', 'sma_adjustments.product_id = sma_products.id', 'left')
	     ...
	     function stockSheet($user_id)
    {
     ...
     // $this->sma->checkPermissions('index');
        ...
        //}
        //$this->form_validation->set_rules('sdate', lang("Date"), 'required');
       
       // if ($this->form_validation->run() == true) {
            
            $data = array('sdate' => $this->input->get('sdate'),);
			$sdate = date('Y-m-d',strtotime("yesterday")).' 12:00:00';
			$edate = $this->input->get('sdate').' 11:59:59';
			
        //} else{
          //  $this->session->set_flashdata('error', (validation_errors() ? validation_errors() : $this->session->flashdata('error')));
          //  redirect("reports");
       // }
		// BAR ONLY STOCK - Warehouse bar //CATEGORY NON-ALCOHOLIC AND ALCOHOLIC
		$result=$this->db->query("SELECT * from (SELECT code,name,SUM(saleqty) as saleqty,SUM(purchaseqty) AS purchaseqty,SUM(adjstqty) AS adjstqty,SUM(saleopqty) as saleopqty,SUM(purchaseopqty) as purchaseopqty,SUM(adjstopqty) as adjstopqty,id,unit,category_id,subcategory_id FROM (
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty, sma_products.category_id,sma_products.subcategory_id FROM sma_products WHERE sma_products.category_id < '57' AND sma_products.category_id <> '53'  UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
...
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
...
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,SUM(IF(sma_adjustments.type='subtraction',sma_adjustments.quantity*-1,sma_adjustments.quantity)) as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
...
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty,0 as purchaseqty,0 as adjstqty, SUM(COALESCE(sma_sale_items.quantity, 0 )) as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
LEFT JOIN `sma_sale_items` ON `sma_products`.`id` = `sma_sale_items`.`product_id`
...
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as adjstqty,0 as saleopqty,SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
...
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,SUM(IF(sma_adjustments.type='subtraction',sma_adjustments.quantity*-1,sma_adjustments.quantity)) as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
LEFT JOIN `sma_adjustments` ON `sma_products`.`id` = `sma_adjustments`.`product_id`
...
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`portion1` as id,unit,sma_products.portion1 as code, sma_products.name ,SUM(COALESCE(sma_products.portion1qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion1`   UNION
SELECT `sma_products`.`portion2`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion2qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion2`  UNION 
SELECT `sma_products`.`portion3`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion3qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion3`  UNION 
SELECT `sma_products`.`portion4`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion4qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion4`  UNION
SELECT `sma_products`.`portion5`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion5qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion5`  UNION 
SELECT `sma_products`.`portion6`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion6qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion6`  UNION
SELECT `sma_products`.`portion7`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion7qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '32' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion7`) T6 GROUP BY `id`)T8 WHERE id > 0 ORDER BY T8.subcategory_id,T8.id,T8.name   ")->result();

              // die($register_open_time);
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['sheet'] = $result;
            $this->data['user_id'] = $user_id;
             // $this->data['loggeduser'] =  $this->pos_model->getUserByID($user_id);
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'reports/newstocksheet', $this->data);
        //}
    }
	     function stockSheetmngnt($user_id)
    {
     ...
     // $this->sma->checkPermissions('index');
       ...
       // }
		// BAR ONLY STOCK - Warehouse bar //CATEGORY NON-ALCOHOLIC AND ALCOHOLIC
		$result=$this->db->query("SELECT * from (SELECT code,price,name,SUM(saleqty) as saleqty,SUM(purchaseqty) AS purchaseqty,SUM(adjstqty) AS adjstqty,SUM(saleopqty) as saleopqty,SUM(purchaseopqty) as purchaseopqty,SUM(adjstopqty) as adjstopqty,id,unit,category_id,subcategory_id FROM (
...
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
LEFT JOIN `sma_purchases` ON `sma_purchases`.`id` = `sma_purchase_items`.`purchase_id`  WHERE sma_purchase_items.warehouse_id = '$warehouseid' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_purchases.date >= '$sdate' AND sma_purchases.date <= '$edate' AND sma_purchases.date > '2022-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code,price, sma_products.name ,0 as saleqty, SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
LEFT JOIN `sma_transfers` ON `sma_transfers`.`id` = `sma_purchase_items`.`transfer_id`  WHERE sma_purchase_items.warehouse_id = '$warehouseid' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_transfers.date >= '$sdate' AND sma_transfers.date <= '$edate' AND sma_transfers.date > '2022-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION
...
SELECT `sma_products`.`portion7`,unit,sma_products.code,price, sma_products.name ,SUM(COALESCE(sma_products.portion7qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as adjstopqty,sma_products.category_id,sma_products.subcategory_id FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '$warehouseid' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion7`) T6 GROUP BY `id`)T8 WHERE id > 0 ORDER BY T8.subcategory_id,T8.id,T8.name   ")->result();
        ...
        //}
		     ...
		     function storeStockSheet($user_id)
    {
     ...
     // $this->sma->checkPermissions('index');
       ...
       // }
		// BAR ONLY STOCK - Warehouse bar //CATEGORY NON-ALCOHOLIC AND ALCOHOLIC
$result=$this->db->query("SELECT * from (SELECT code,name,SUM(saleqty) as saleqty,SUM(purchaseqty) AS purchaseqty,SUM(stockoutqty) AS stockoutqty,SUM(adjstqty) AS adjstqty,SUM(saleopqty) as saleopqty,SUM(purchaseopqty) as purchaseopqty,SUM(stockoutopqty) AS stockoutopqty,SUM(adjstopqty) as adjstopqty,id,unit FROM (
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as stockoutqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty, 0 as adjstopqty FROM sma_products WHERE sma_products.category_id < '57' AND sma_products.category_id <> '53'  UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockinqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_sale_items` ON `sma_products`.`id` = `sma_sale_items`.`product_id`
LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id` WHERE sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-09-20 05:00:00' GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseqty,0 as stockoutqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
LEFT JOIN `sma_purchases` ON `sma_purchases`.`id` = `sma_purchase_items`.`purchase_id`  WHERE sma_purchase_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_purchases.date >= '$sdate' AND sma_purchases.date <= '$edate' AND sma_purchases.date > '2021-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty,  0 as purchaseqty,SUM(COALESCE(sma_purchase_items.quantity, 0 )) as stockoutqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
LEFT JOIN `sma_transfers` ON `sma_transfers`.`id` = `sma_purchase_items`.`transfer_id`  WHERE sma_transfers.from_warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_transfers.date >= '$sdate' AND sma_transfers.date <= '$edate' AND sma_transfers.date > '2021-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as stockoutqty,SUM(IF(sma_adjustments.type='subtraction',sma_adjustments.quantity*-1,sma_adjustments.quantity)) as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_adjustments` ON `sma_products`.`id` = `sma_adjustments`.`product_id`
WHERE sma_adjustments.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_adjustments.date >= '$sdate' AND sma_adjustments.date <= '$edate' AND sma_adjustments.date >  '2021-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, SUM(COALESCE(sma_sale_items.quantity, 0 )) as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_sale_items` ON `sma_products`.`id` = `sma_sale_items`.`product_id`
LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id` WHERE sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_sales.date < '$sdate' AND  sma_sales.date > '2021-09-20 05:00:00'  GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as stockoutqty,0 as adjstqty,0 as saleopqty,SUM(COALESCE(sma_purchase_items.quantity, 0 )) as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
LEFT JOIN `sma_purchases` ON `sma_purchases`.`id` = `sma_purchase_items`.`purchase_id`  WHERE sma_purchase_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_purchases.date < '$sdate' AND sma_purchases.date >'2021-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as stockoutqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,SUM(COALESCE(sma_purchase_items.quantity, 0 )) as stockoutopqty,0 as adjstopqty FROM sma_products 
LEFT JOIN `sma_purchase_items` ON `sma_products`.`id` = `sma_purchase_items`.`product_id`
LEFT JOIN `sma_transfers` ON `sma_transfers`.`id` = `sma_purchase_items`.`transfer_id`  WHERE sma_transfers.from_warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53' AND  sma_transfers.date < '$sdate' AND sma_transfers.date >'2021-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION 
SELECT `sma_products`.`id`,unit,sma_products.code, sma_products.name ,0 as saleqty, 0 as purchaseqty,0 as stockoutqty,0 as adjstqty,0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,SUM(IF(sma_adjustments.type='subtraction',sma_adjustments.quantity*-1,sma_adjustments.quantity)) as adjstopqty FROM sma_products 
LEFT JOIN `sma_adjustments` ON `sma_products`.`id` = `sma_adjustments`.`product_id`
WHERE sma_adjustments.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND  sma_adjustments.date < '$sdate' AND sma_adjustments.date >'2021-09-20 05:00:00'
GROUP BY `sma_products`.`id` UNION
SELECT `sma_products`.`portion1` as id,unit,sma_products.portion1 as code, sma_products.name ,SUM(COALESCE(sma_products.portion1qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion1`   UNION
SELECT `sma_products`.`portion2`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion2qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion2`  UNION 
SELECT `sma_products`.`portion3`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion3qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion3`  UNION 
SELECT `sma_products`.`portion4`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion4qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion4`  UNION
SELECT `sma_products`.`portion5`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion5qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion5`  UNION 
SELECT `sma_products`.`portion6`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion6qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion6`  UNION
SELECT `sma_products`.`portion7`,unit,sma_products.code, sma_products.name ,SUM(COALESCE(sma_products.portion7qty*sma_sale_items.quantity, 0 )) as saleqty,0 as purchaseqty,0 as stockoutqty,0 as adjstqty, 0 as saleopqty,0 as purchaseopqty,0 as stockoutopqty,0 as adjstopqty FROM sma_sale_items LEFT JOIN  `sma_products` ON sma_sale_items.product_id = sma_products.id  LEFT JOIN `sma_sales` ON `sma_sales`.`id` = `sma_sale_items`.`sale_id`  WHERE sma_sale_items.product_id IN (SELECT id FROM sma_products WHERE unit = 'cocktail') AND sma_sale_items.warehouse_id = '31' AND sma_products.category_id < '57' AND sma_products.category_id <> '53'  AND sma_sales.date >= '$sdate' AND sma_sales.date <= '$edate' AND sma_sales.date > '2021-11-13 05:00:00'  GROUP BY `sma_products`.`portion7`) T6 GROUP BY `id`)T8 WHERE id > 0  ")->result();
            ...
            $this->data['user_id'] = $user_id;
             // $this->data['loggeduser'] =  $this->pos_model->getUserByID($user_id);
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'reports/storestocksheet', $this->data);
        //}
    }
	...
	// 			COALESCE( PCosts.purchasedQty, 0 ) as PurchasedQty,
	// 			COALESCE( PSales.soldQty, 0 ) as SoldQty,
    //             (COALESCE( PCosts.purchasedQty, 0 ) - COALESCE( PSales.soldQty, 0 )) as RemQty,
	// 			COALESCE( PCosts.totalPurchase, 0 ) as TotalPurchase,
    ...
    //             $this->excel->getActiveSheet()->SetCellValue('E1', lang('remaining'));
    ...
    //                 $this->excel->getActiveSheet()->SetCellValue('E' . $row, $data_row->RemQty);
    //                 $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->TotalPurchase);
    //                 $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->TotalSales);
    //                 $this->excel->getActiveSheet()->SetCellValue('H' . $row, $profit);
    //                 $pQty += $data_row->PurchasedQty;
    //                 $sQty += $data_row->SoldQty;
    //                 $rQty += $data_row->RemQty;
    //                 $pAmt += $data_row->TotalPurchase;
    //                 $sAmt += $data_row->TotalSales;
    //                 $pl += $profit;
    //                 $row++;
    ...
    //             $this->excel->getActiveSheet()->SetCellValue('E' . $row, $rQty);
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
          ...
          $this->datatables
                ->select("sales.date, sales.id as reference_no,sales.chef as biller, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, '__', " . $this->db->dbprefix('sale_items') . ".quantity) SEPARATOR '___') as iname, grand_total, paid, (grand_total-paid) as balance, IF(grand_total<=paid,'paid','due') payment_status,pmethod as paid_by,CONCAT(mpesa_transaction_no,'',cost_center_no,'',cheque_no,'',cc_no) as transaction_no", FALSE)   //paid_by,transaction_id
    ...
    // function getStaffSalesReport($pdf = NULL, $xls = NULL)
    // {
    //     $this->sma->checkPermissions('sales', TRUE);
    //     if ($this->input->get('product')) {
    //         $product = $this->input->get('product');
    ...
    //         $this->db
    //             ->select('product_name, sma_sales.pmethod ,SUM(sma_sale_items.quantity) AS QTY')
    //             ->join('sale_items', 'sale_items.sale_id = sales.id', 'left')
    ...
    //             ->group_by('product_id')
    //             ->order_by('products.category_id', 'asc');
    //             //SELECT SUM(grand_total) FROM `sma_sales` WHERE sma_sales.pos=1 and date BETWEEN '2023-06-15 10:00:00' AND '2023-06-16 09:59:50'
    ...
    function getPaymentsReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("" . $this->db->dbprefix('payments') . ".date, " . $this->db->dbprefix('payments') . ".reference_no as payment_ref, " . $this->db->dbprefix('sales') . ".reference_no as sale_ref, " . $this->db->dbprefix('purchases') . ".reference_no as purchase_ref,sales.biller_id,paid_by, amount, type")
                ->from('payments')
            ...
            if ($start_date) {
                ...
                $this->db->where($this->db->dbprefix('payments').'.date BETWEEN "' . $start_date . ' 00:01:00" and "' . $end_date . ' 23:59:00"');
            }
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select($this->db->dbprefix('payments') . ".date, " . $this->db->dbprefix('payments') . ".reference_no as payment_ref, " . $this->db->dbprefix('sales') . ".id as sale_ref, " . $this->db->dbprefix('purchases') . ".reference_no as purchase_ref,sma_companies.name, paid_by, amount, type,sma_users.first_name")
                ->from('payments')
                ->join('sales', 'payments.sale_id=sales.id', 'left')
                ->join('purchases', 'payments.purchase_id=purchases.id', 'left')
            ...
            if ($start_date) {   
                ...
                $this->datatables->where($this->db->dbprefix('payments').'.date BETWEEN "' . $start_date . ' 00:01:00" and "' . $end_date . ' 23:59:00"');
            }
...
$sales=$this->reports_model->getCustomerDueSales($user_id);
     ...
     $company=$this->companies_model->getCompanyByID($user_id);
        ...
        $html='<table width="98%" border="1" style="margin-left:10px;border-collapse:collapse">
            <tr><th width="30%"><img src="' . site_url() . 'assets/uploads/logos/logo3.png" alt="' . SITE_NAME . '"  /></th><th style="text-align:left" colspan="5"><u>UNPAID CUSTOMER BILL:'.strtoupper($company->name).'</u></th></tr>
