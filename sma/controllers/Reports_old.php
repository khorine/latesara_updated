    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->datatables
                ->select("
                    sales.date,
                    sales.id as reference_no,
                    sales.chef as biller,
                    customer,
                    ...
                    IF(grand_total <= paid, 'paid', 'due') as payment_status,
                    pmethod as paid_by,
                    CONCAT(mpesa_transaction_no, '', cost_center_no, '', cheque_no, '', cc_no) as transaction_no", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id = sales.id', 'left')
