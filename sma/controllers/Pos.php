...
class Pos extends MY_Controller
{
  ...
  function poslogin(){
      ...
      // print_r($userid.' '.$password);
       $res['msg']=0;
        if($this->pos_model->poslogin($userid, $password)){
         $res['msg']=1;   
        }else{
          $res['msg']=0;     
        }
    ...
    function index($sid = NULL)
    {
		...
		$this->form_validation->set_rules('table_no', $this->lang->line("Table No Required"), 'required');
        $this->form_validation->set_rules('biller', $this->lang->line("biller"), 'required');
		//$this->form_validation->set_rules('count_cust', $this->lang->line("No of Customers"), 'required');
//        $this->form_validation->set_rules('cashiers', 'Cashier', 'required');
//        $this->form_validation->set_rules('chefs', 'Chefs', 'required');
        if ($this->form_validation->run() == true) {
			...
			$table_id = $this->input->post('table_no');
            $customer_id = $this->input->post('customer');
            $biller_id = $this->input->post('biller');
			$count_cust = $this->input->post('count_cust');
            //die($biller_id."fdfd");
            ...
            //die(print_r($biller_id));
            $biller_details = $this->site->getCompanyByID($biller_id);
            $biller =$biller_details->company.'-'.$biller_details->name;
            
            //additons for chef 
            
             $chef_details = $this->site->getUser($chef_id);
             $chef = $chef_details->first_name.' '.$chef_details->last_name;
            
            //additons for cashier
             $cashier_details = $this->site->getUser($cashier_id);
             $cashier = $cashier_details->first_name." ". $cashier_details->last_name;
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            $this->data['chefs'] = $this->site->getUsers(6);
            $this->data['cashiers'] = $this->site->getUsers(9);
            
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['tax_rates'] = $this->site->getAllTaxRates();
            $this->data['rooms'] = $this->site->getAllRooms();
			$this->data['tables'] = $this->site->getTables($this->session->userdata('user_id'));
    ...
    function view_bill()
    {
     ...
     function view_complete($sale_id = NULL, $modal = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        $this->data['customer'] = $this->pos_model->getCompanyByID($customer_id);
          $this->data['soldby'] = $this->pos_model->getUserByID($inv->created_by);
    ...
    function close_register($user_id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['mpesasales'] = $this->pos_model->getRegisterMPESASales($register_open_time);
            $this->data['costcentersales'] = $this->pos_model->getRegisterCostCenterSales($register_open_time);
...
//            }
    ...
    function billview($sale_id = NULL, $modal = NULL)
    {
        $this->sma->checkPermissions('index');
        if ($this->input->get('id')) {
            $sale_id = $this->input->get('id');
        ...
        $this->data['modal'] = $modal;
        $this->data['page_title'] = $this->lang->line("invoice");
        $this->load->view($this->theme . 'pos/viewc', $this->data);
    }
	function viewsalepos($modal = NULL)
    {
        $this->sma->checkPermissions('index');
       if ($this->input->get('startdate')) {
        ...
        $this->data['modal'] = $modal;
        $this->data['page_title'] = $this->lang->line("invoice");
        $this->load->view($this->theme . 'pos/viewsaleprdct', $this->data);
    }
	function viewsalesumm($modal = NULL)
    {
      ...
      //$this->load->view($this->theme . 'pos/viewsmrpt', $this->data);
		$this->load->view($this->theme . 'pos/viewsalesrpt', $this->data);
    }
	function printcombinebill($modal = NULL)
    {
        $this->sma->checkPermissions('index');
       if ($this->input->get('tableid')) {
        ...
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['message'] = $this->session->flashdata('message');
       	$this->data['bill'] = $this->pos_model->getcombinedtable_bill($tableid,$user_id,$sdate,$edate);
	  $this->data['billnos'] = $this->pos_model->getunique_billno($tableid,$user_id,$sdate,$edate);
		$this->data['start'] = $sdate;
		$this->data['end'] = $edate;
               $this->data['pos'] = $this->pos_model->getSetting();
        $this->data['modal'] = $modal;
        $this->data['page_title'] = $this->lang->line("invoice");
      //$this->load->view($this->theme . 'pos/viewsmrpt', $this->data);
		$this->load->view($this->theme . 'pos/combinebill', $this->data);
    }
    ...
    function today_sale()
    {
        ...
        $this->data['mpesasales'] = $this->pos_model->getTodayMPESASales();
        $this->data['costcentersales'] = $this->pos_model->getTodayCostCenterSales();
    ...
    function settings()
    {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->updateSetting($data)) {
            ...
            //changes for chefs & cashiers
            $this->data['chefs'] = $this->site->getUsers(6);
            $this->data['cashiers'] = $this->site->getUsers(5);
	...
	function getDeletedSales($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('deleted_sales').".id as id, date, reference_no, biller,created_by, customer, grand_total, paid, (grand_total-paid) as balance, payment_status, companies.email as cemail")
                ->from('deleted_sales')
                ->join('companies', 'companies.id=deleted_sales.customer_id', 'left')
                ->where('warehouse_id', $warehouse_id)
                ->group_by('deleted_sales.id');
        } else {
            $this->datatables
                ->select($this->db->dbprefix('deleted_sales').".id as id, date, reference_no, biller,created_by, customer, grand_total, paid, (grand_total-paid) as balance, payment_status, companies.email as cemail")
                ->from('deleted_sales')
	...
	function deletedsales($warehouse_id = NULL)
    {
        $this->sma->checkPermissions('index');
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('pos'), 'page' => lang('pos')), array('link' => '#', 'page' => lang('deleted_pos_sales')));
        $meta = array('page_title' => lang('deleted_pos_sales'), 'bc' => $bc);
        $this->page_construct('pos/deletedsales', $meta, $this->data);
    }
...
WHERE  ('$sdate' >= sma_roomstats.check_in AND  '$sdate' < sma_roomstats.check_out) ) AS T1 RIGHT JOIN sma_rooms ON sma_rooms.id = T1.id")->result();
    ...
    function add_payment($id = NULL)
    {
       // $this->sma->checkPermissions('payments', true);
		...
		if($this->input->post('paid_by') == 'rooms'){
			$this->form_validation->set_rules('room_no', lang("room_no"), 'required');
			$custid =$this->input->post('room_cust');
			$amount = $this->input->post('amount-paid');
			$bill_change = ($this->input->post('whole-amount-paid'))-($this->input->post('amount-paid'));
		}
		
		if($this->input->post('paid_by') == 'compli'){
			$this->form_validation->set_rules('complimentary_rmk', lang("Complimentary_Remark"), 'required');
			$remark= $this->input->post('complimentary_rmk');
			$amount = $this->input->post('amount-paid');
			$bill_change = ($this->input->post('whole-amount-paid'))-($this->input->post('amount-paid'));
		}
		if($this->input->post('paid_by') == 'nonchrg'){
			$this->form_validation->set_rules('nonchrg_rmk', lang("Non_Chargeable_Remark"), 'required');
			$remark= $this->input->post('nonchrg_rmk');
			$amount = 0;
			$bill_change = 0;
		}
		if($this->input->post('paid_by') == 'staff'){
			$this->form_validation->set_rules('staff_no_1', lang("Select Staff"), 'required');
			$remark= $this->input->post('staff_no_1');
			$amount = $this->input->post('amount-paid');
			$bill_change = ($this->input->post('whole-amount-paid'))-($this->input->post('amount-paid'));
		}
		else{
...
WHERE  ('$sdate' >= sma_roomstats.check_in AND  '$sdate' < sma_roomstats.check_out) ) AS T1 RIGHT JOIN sma_rooms ON sma_rooms.id = T1.id")->result();
	 ...
	 function combine_bill($id = NULL)
    {
       // $this->sma->checkPermissions('payments', true);
		...
		//die($this->input->post('sale_id'));
        $this->form_validation->set_rules('reference_no', lang("reference_no"), 'required');
        $this->form_validation->set_rules('bill_no', lang("bill_no"), 'required');
        if ($this->form_validation->run() == true) {
            if ($this->Owner || $this->Admin) {
                $date = $this->sma->fld(trim($this->input->post('date')));
            } else {
        ...
        } elseif ($this->input->post('combine_bill')) {
            $this->session->set_flashdata('error', validation_errors());
            redirect($_SERVER["HTTP_REFERER"]);
        }


        if ($this->form_validation->run() == true && $msg = $this->pos_model->combineBill($payment)) {
			//die(print_r($msg));
            if ($msg) {
                if ($msg['status'] == 0) {
                    $error = '';
            ...
            redirect("pos/billview/".$msg['sale_id']);
                }
            } else {
                $this->session->set_flashdata('error', lang("payment_failed"));
            }
...
$result=$this->db->query("select name from sma_companies where customer_group_name like '%corporate%' ")->result();
  ...
  $this->data["costcenter"]= $result;
			...
			//die("SELECT sma_sales.id as id,date,reference_no, biller, customer, grand_total, paid, (grand_total-paid) as balance, payment_status
				//			FROM sma_sales WHERE id <> '$id' AND payment_status='due'  GROUP BY sma_sales.id");
			//print_r($q);
			$this->data['bills'] = $q;
            $this->data['inv'] = $sale;
            $this->data['payment_ref'] = $this->site->getReference('pay');
            ...
            $this->load->view($this->theme . 'pos/combine_bill', $this->data);
        }
    }
    ...
    function install_update($file, $m_version, $version)
    {
     ...
     function closes_register($user_id)
    {
      ...
      $this->sma->checkPermissions('index');
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            if ($this->Owner || $this->Admin) {
                $user_register = $user_id ? $this->pos_model->registerData($user_id) : NULL;
                $register_open_time = date("Y-m-d H:i:s", strtotime("yesterday 12:00"));//$user_register ? $user_register->date : $this->session->userdata('register_open_time');
                $this->data['cash_in_hand'] = $user_register ? $user_register->cash_in_hand : NULL;
                $this->data['register_open_time'] = $user_register ? $register_open_time : NULL;
            } else {
                $register_open_time =date("Y-m-d H:i",strtotime("today 1:00"));// $this->session->userdata('register_open_time');
           ...
           // die($register_open_time);
            ...
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'pos/zed', $this->data);
        }
    }
