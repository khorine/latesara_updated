    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("products/add_kitchen", $attrib)
                ?>
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group standard">
                        ...
                        <div
                            class="input-group"> <?= form_input('alert_quantity', (isset($_POST['alert_quantity']) ? $_POST['alert_quantity'] : ($product ? $this->sma->formatNumber($product->alert_quantity) : '')), 'class="form-control tip" id="alert_quantity"') ?>
                            ...
                            <input type="checkbox" name="track_quantity" id="track_quantity"
                                   value="1" <?= ($product ? (isset($product->track_quantity) ? 'checked="checked"' : '') : 'checked="checked"') ?>>
                                       ...
                                       <!-- Modifications for a portioning  -->
<!--                      <div class="form-group">
                            <input type="checkbox" class="checkbox" name="portions_cb"
                                   id="portions_cb" <?//= $this->input->post('portions_cb') || $product_portions ? 'checked="checked"' : ''; ?>><label
                                for="portions_cb"
                                class="padding05"><?//= "Is Kitchen Product"; ?></label> <?//= " i.e. Has Portions of Other Products"; ?>
                      </div>-->
                     <div class="form-group standard">
                        <?= "<b>Portions</b>" ?>
                        <button type="button" class="btn btn-primary btn-xs" id="addPortion"><i class="fa fa-plus"></i>
                        </button><br></br>
                        <div class="row" id="portion-con">
                            <div class="col-md-8 col-sm-8 col-xs-8">
                                <?php
                                  echo form_input('portion', (isset($_POST['portion']) ? $_POST['portion'] : ''), 'class="form-control ' . ($product ? '' : '') . '" id="' . ($product && ! empty($product->portion1) ? 'portion1' : 'portion') . '" placeholder="Select Product" style="width:100%;"')
                                ?></div>
                            <div
                                class="col-md-4 col-sm-4 col-xs-4"><?= form_input('portion_qty', (isset($_POST['portion_qty']) ? $_POST['portion_qty'] : ""), 'class="form-control tip" id="portion_qty" placeholder="Portion Qty"') ?></div>
                        </div>
                        <div id="ex-portions"></div>
                        <input type="hidden" name="iskitchen" value=1>
                    </div>
<!--   Modifications for a portioning    -->
                    <div class="form-group standard">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var po = 2;
        $('#addPortion').click(function () {
            if (po <= 13) {
                //alert();
                $('#portion_1').select2('destroy');
                var html = '<div style="clear:both;height:15px;"></div><div class="row"><div class="col-md-8 col-sm-8 col-xs-8"><input type="hidden" name="portion_' + po + '", class="form-control" id="portion_' + po + '" placeholder="<?= lang("select") . " Product " ?>" style="width:100%;display: block !important;" /></div><div class="col-md-4 col-sm-4 col-xs-4"><input type="text" name="portion_' + po + '_qty" class="form-control tip" id="portion_' + po + '_qty" placeholder="<?= "Portion Qty" ?>" /></div></div>';
                $('#ex-portions').append(html);
                var por = $('#portion_' + po);
                portions(por);
                po++;
            } else {
...
//Modification for aportioning Start


        <?php if ($product->portion1) { ?>
        select_portion('portion1', "<?= $product->portion1; ?>");
        $('#portion_qty').val("<?= $product->portion1qty == 0 ? '' : $this->sma->formatDecimal($product->portion1qty); ?>");
        <?php } ?>
        <?php if ($product->portion2) { ?>
        $('#addPortion').click();
        select_portion('portion_2', "<?= $product->portion2; ?>");
        $('#portion_2_qty').val("<?= $product->portion2qty == 0 ? '' : $this->sma->formatDecimal($product->portion2qty); ?>");
        <?php } ?>
        <?php if ($product->portion3) { ?>
        $('#addPortion').click();
        select_portion('portion_3', "<?= $product->portion3; ?>");
        $('#portion_3_qty').val("<?= $product->portion3qty == 0 ? '' : $this->sma->formatDecimal($product->portion3qty); ?>");
        <?php } ?>
        <?php if ($product->portion4) { ?>
        $('#addPortion').click();
        select_portion('portion_4', "<?= $product->portion4; ?>");
        $('#portion_4_qty').val("<?= $product->portion4qty == 0 ? '' : $this->sma->formatDecimal($product->portion4qty); ?>");
        <?php } ?>
        <?php if ($product->portion5) { ?>
        $('#addPortion').click();
        select_portion('portion_5', "<?= $product->portion5; ?>");
        $('#portion_5_qty').val("<?= $product->portion5qty == 0 ? '' : $this->sma->formatDecimal($product->portion5qty); ?>");
        <?php } ?>
                  <?php if ($product->portion6) { ?>
        $('#addPortion').click();
        select_portion('portion_6', "<?= $product->portion6; ?>");
        $('#portion_6_qty').val("<?= $product->portion6qty == 0 ? '' : $this->sma->formatDecimal($product->portion6qty); ?>");
        <?php } ?>
            <?php if ($product->portion7) { ?>
        $('#addPortion').click();
        select_portion('portion_7', "<?= $product->portion7; ?>");
        $('#portion_7_qty').val("<?= $product->portion7qty == 0 ? '' : $this->sma->formatDecimal($product->portion7qty); ?>");
        <?php } ?>
            <?php if ($product->portion8) { ?>
        $('#addPortion').click();
        select_portion('portion_8', "<?= $product->portion8; ?>");
        $('#portion_8_qty').val("<?= $product->portion8qty == 0 ? '' : $this->sma->formatDecimal($product->portion8qty); ?>");
        <?php } ?>
            <?php if ($product->portion9) { ?>
        $('#addPortion').click();
        select_portion('portion_9', "<?= $product->portion9; ?>");
        $('#portion_9_qty').val("<?= $product->portion9qty == 0 ? '' : $this->sma->formatDecimal($product->portion9qty); ?>");
        <?php } ?>
            <?php if ($product->portion10) { ?>
        $('#addPortion').click();
        select_portion('portion_10', "<?= $product->portion10; ?>");
        $('#portion_10_qty').val("<?= $product->portion10qty == 0 ? '' : $this->sma->formatDecimal($product->portion10qty); ?>");
        <?php } ?>
            <?php if ($product->portion11) { ?>
        $('#addPortion').click();
        select_portion('portion_11', "<?= $product->portion11; ?>");
        $('#portion_11_qty').val("<?= $product->portion11qty == 0 ? '' : $this->sma->formatDecimal($product->portion11qty); ?>");
        <?php } ?>
            <?php if ($product->portion12) { ?>
        $('#addPortion').click();
        select_portion('portion_12', "<?= $product->portion12; ?>");
        $('#portion_12_qty').val("<?= $product->portion12qty == 0 ? '' : $this->sma->formatDecimal($product->portion12qty); ?>");
        <?php } ?>
            <?php if ($product->portion13) { ?>
        $('#addPortion').click();
        select_portion('portion_13', "<?= $product->portion13; ?>");
        $('#portion_13_qty').val("<?= $product->portion13qty == 0 ? '' : $this->sma->formatDecimal($product->portion13qty); ?>");
        <?php } ?>
        function select_portion(id, v) {
            //alert('Myw');
            $('#' + id).val(v).select2({
                minimumInputLength: 1,
                data: [],
                initSelection: function (element, callback) {
                    $.ajax({
                        type: "get", async: false,
                        url: "<?= site_url('products/getProducts') ?>/" + $(element).val(),
                        dataType: "json",
                        success: function (data) {
                            callback(data[0]);
                        }
                ...
                ajax: {
                    url: site.base_url + "products/portions",
                    dataType: 'json',
                    quietMillis: 15,
                    data: function (term, page) {
                        return {
            ...
            });//.select2("val", "<?= $product->portion1; ?>");
        }




//Modification of aportioning End
        var whs = $('.wh');
        $.each(whs, function () {
