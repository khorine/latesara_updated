 ...
 <h4 ><b> FOOD COSTING</b></h4>
           ...
           // echo "Chef" . ": " . $inv->chef . "</br>";
            ...
            <table class="table table-striped table-condensed" border="1">
				...
				<th align="centre" width="150px">Transfer Cost</th>
				<th align="centre" width="150px">Purchase Cost</th>
				<th align="centre" width="150px">NC Cost</th>
				<th align="centre" width="150px">Total Sales</th>
				<th align="centre" width="150px">Food Costing</th>
...
for ( $i = $startTime; $i <= $endTime; $i = $i + 86400 ) {
  ...
  $transfercosts =0; $purchsasecosts =0; $nonchrgcosts=0; $totalsaleamt=0;
foreach($transfercost as $transfercst){
	if($transfercst->transferdate ==  date( 'Y-m-d', $i )){
		$transfercosts = $transfercst->transfercost;
	}
}
foreach($purchsase as $purchsasecst){
	if($purchsasecst->purchasedate ==  date( 'Y-m-d', $i )){
		$purchsasecosts = $purchsasecst->purchsasecost;
	}
}

foreach($nonchrg as $nonchrgcst){
	if($nonchrgcst->ncdate ==  date( 'Y-m-d', $i )){
		$nonchrgcosts = $nonchrgcst->ncsale;
	}
}

foreach($totalsales as $totalsale){
	if($totalsale->tdate ==  date( 'Y-m-d', $i )){
		$totalsaleamt = $totalsale->sale;
	}
}
$r++;
$foodcost = ($transfercosts + $purchsasecosts - $nonchrgcosts)/$totalsaleamt;
  echo "<tr><td>$r</td><td>". date( 'd-m-Y', $i )."</td><td>".number_format($transfercosts)." </td><td>".number_format($purchsasecosts)."</td><td>".number_format($nonchrgcosts)." </td><td>".number_format($totalsaleamt)."</td><td>".round($foodcost,2)." </td></tr>";
}
