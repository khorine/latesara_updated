...
<!--            <h3 style="text-transform:uppercase;"><b>$inv->id$biller->company != '-' ? $biller->company : $biller->name; ?></b></h3>-->
 ...
 <h4 ><b> Bar/Restaurant Sales </b></h4>
           ...
           // echo "Chef" . ": " . $inv->chef . "</br>";
            echo "<b> Between  ".lang("date") . ": " . $this->sma->hrld($start) . " AND " . $this->sma->hrld($end) . "</p></b>";
           
            ?>
            <div style="clear:both;"></div>
            <table class="table table-striped table-condensed" border="1">
                <tbody>
				<tr><td style="text-align:center" colspan="3">Bar</td></tr>
				<tr><td style="text-align:center" colspan="2"></td><td align="centre">Amount</td></tr>
                <?php
                $r = 1;
                $tax_summary = array();
				$totalbar = 0;
                foreach ($barsales as $row) {
					if($row->paid_by=='compli' ||$row->paid_by=='nonchrg' ){
						$totalbar = $totalbar +0;
					}else{
					$totalbar = $totalbar + $row->amnt;
					}
                   if($row->paid_by=='cash'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Cash'  ;
                  
					echo '<td>&nbsp' . number_format($row->amnt) . '</td></tr>';
				   } 
				   elseif($row->paid_by=='mpesa'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Mobile money'  ;
                  
					echo '<td>&nbsp' . number_format($row->amnt) . '</td></tr>';
				   }
				   elseif($row->paid_by=='nonchrg'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Non Chargeable'  ;
                  
					echo '<td>-&nbsp</td></tr>';
				   }
				   elseif($row->paid_by=='compli'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Reception'  ;
                  
					echo '<td>&nbsp' . number_format($row->amnt) . '</td></tr>';
				   }
				   elseif($row->paid_by=='Cheque'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Cheque:'  ;
                  
					echo '<td>&nbsp' . number_format($row->amnt) . '</td></tr>';
				   }else{
                    echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp;' .$row->paid_by  ;
			...
			<tr><td style="text-align:center;border-color: black !important;" border="1" colspan="2"><b>Bar Total </b></td><td align="centre" style="border-color: black !important;" border="1"><?php echo number_format($totalbar) ; ?></td></tr>
				...
				$totalrest = 0;
                foreach ($restsales as $roww) {
					if($roww->paid_by=='compli' ||$roww->paid_by=='nonchrg' ){
						$totalrest = $totalrest + 0;
					}else{
						$totalrest = $totalrest + $roww->amnt;
					}
					
                   if($roww->paid_by=='cash'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Cash'  ;
                 
					echo '<td>&nbsp' . number_format($roww->amnt) . '</td></tr>';
				   } 
				   elseif($roww->paid_by=='mpesa'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Mobile money'  ;
                  
					echo '<td>&nbsp' . number_format($roww->amnt) . '</td></tr>';
				   }
				   elseif($roww->paid_by=='nonchrg'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Non Chargeable'  ;
                  
					echo '<td>-&nbsp</td></tr>';
				   }
				   elseif($roww->paid_by=='compli'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Reception'  ;
                  
					echo '<td>&nbsp' . number_format($roww->amnt) . '</td></tr>';
				   }
				   elseif($roww->paid_by=='Cheque'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Cheque:'  ;
                  
					echo '<td>&nbsp' . number_format($roww->amnt) . '</td></tr>';
				   }else{
                    echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp;' .$roww->paid_by  ;
				...
				<tr><td style="text-align:center;border-color: black !important;" colspan="3"  border="1">Delivery/Room Service</td></tr>
				<tr><td style="text-align:center" colspan="2"></td><td align="centre">Amount</td></tr>
                <?php
                $r = 1;
               
				$totalservice = 0;
                foreach ($servicesales as $rowww) {
					if($rowww->paid_by=='compli' ||$rowww->paid_by=='nonchrg' ){
						$totalservice = $totalservice + 0;
					}else{
						$totalservice = $totalservice + $rowww->amnt;
					}
					
                   if($rowww->paid_by=='cash'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Cash'  ;
                 
					echo '<td>&nbsp' . number_format($rowww->amnt) . '</td></tr>';
				   } 
				   elseif($rowww->paid_by=='mpesa'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Mobile money'  ;
                  
					echo '<td>&nbsp' . number_format($rowww->amnt) . '</td></tr>';
				   }
				   elseif($rowww->paid_by=='nonchrg'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Non Chargeable'  ;
                  
					echo '<td>-&nbsp</td></tr>';
				   }
				   elseif($rowww->paid_by=='compli'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Reception'  ;
                  
					echo '<td>&nbsp' . number_format($rowww->amnt) . '</td></tr>';
				   }
				   elseif($rowww->paid_by=='Cheque'){
			echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp; Cheque:'  ;
                  
					echo '<td>&nbsp' . number_format($rowww->amnt) . '</td></tr>';
				   }else{
                    echo '<tr style="font-size:15px"><td colspan="2">#:&nbsp;&nbsp;' .$rowww->paid_by  ;
                  
					echo '<td>&nbsp' . number_format($rowww->amnt) . '</td></tr>';
					                }
				}
                ?>
				<tr><td style="text-align:center;border-color: black !important;" border="1" colspan="2"><b>Services Total </b></td><td align="centre" style="border-color: black !important;" border="1"> &nbsp;&nbsp;<?php echo number_format($totalservice) ; ?></td></tr>
				
				<tr><td style="text-align:center;border-color: black !important;" border="1" colspan="3"></td></tr>
				<tr><td style="text-align:center;border-color: black !important;" border="1" colspan="2"><b>Sub Total </b></td><td align="centre" style="border-color: black !important;" border="1"> &nbsp;&nbsp;<?php echo number_format($totalrest+$totalbar+$totalservice) ; ?></td></tr>
