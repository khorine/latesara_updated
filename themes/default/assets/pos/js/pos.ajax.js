...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		if(java_applet == 1) {
			...
			$("#order_span").append(pos_head1+' Order '+pos_head2);
			$("#bill_span").append(pos_head1+' Bill '+pos_head2);
		...
		//alert();
		if(gtotal<0){
			alert('Discount is More than Price!!! Not Allowed');
			gtotal = parseFloat(((total + invoice_tax)) + shipping);
			$('#gtotal').text(formatMoney(gtotal))
			$('#tds').text('('+formatMoney(0)+') '+formatMoney(0));
		}else{
