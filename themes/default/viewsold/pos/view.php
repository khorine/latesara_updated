...
<div id="wrapper">
 ...
 <h3 style="text-transform:uppercase;"><b>Eastbite Meat Palace LTD</b></h3>
             <h3><b>Receipt No: </b><?= $inv->id; ?></h3>
            <?php
			$data1 = 'RECEIPT NO: '.$inv->id.''. PHP_EOL;
						$this->load->helper('file');
				echo "PIN # - P051615194Q<br>";
				echo "P.O Box 10538 - 00200";
           // echo "<p>" . $biller->address . " " . $biller->city . " " . $biller->postal_code . " " . $biller->state . " " . $biller->country .
                //"<br>" . lang("tel") . ": " . $biller->phone . "<br>";
				//$data2 = $biller->address . " " . $biller->city . " " . $biller->postal_code . " " . $biller->state . " " . $biller->country . PHP_EOL; 
				//$data3 = lang("tel") . ": " . $biller->phone . PHP_EOL; 
            ?>
            <?php
			...
			$data4 = lang("reference_no") . ": " . $inv->reference_no . PHP_EOL; 
           // echo lang("customer") . ": " . $inv->customer . "<br>";
			$data5 = lang("customer") . ": " . $inv->customer . PHP_EOL; 
            //echo "Cashier" . ": " . $inv->cashier . "</br>";
           // echo "Chef" . ": " . $inv->chef . "</br>";
            echo lang("date") . ": " . $this->sma->hrld($inv->date) . "</p>";
			$data6 = lang("date") . ": " . $this->sma->hrld($inv->date) . PHP_EOL;
            ...
            <table class="table table-striped table-condensed">
                ...
                foreach ($rows as $row) {
                    ...
                    $data7 = '#' . $r . ':  ' . product_name($row->product_name) . ($row->variant ? ' (' . $row->variant . ')' : '') . '*' . $row->tax_code . '' . PHP_EOL;
					echo '<tr><td>' . number_format($row->quantity) . ' x ';
                ...
                <tr>
					...
					<?php 
                    $data11 = lang("total")."  ".$this->sma->formatMoney($inv->total + $inv->product_tax). PHP_EOL;
                ...
                <?php
				$data12 = lang("grand_total") . "  ". $this->sma->formatMoney($inv->grand_total) . PHP_EOL;
				}
  ...
  echo '<p>You were served by:'.$soldby->first_name.'&nbsp;'.$soldby->last_name.'</p>';
            ...
            if ($payments) {
                ...
                $data13 = lang("paid_by") . ' : ' . lang($payment->paid_by) . lang("amount") . ': ' . $this->sma->formatMoney($payment->pos_paid == 0 ? $payment->amount : $payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '  ').lang("change") . ': ' . ($payment->pos_balance > 0 ? $this->sma->formatMoney($payment->pos_balance) : 0) . PHP_EOL;
                 ...
                 $data13 = lang("paid_by") . ': ' . lang($payment->paid_by) . lang("amount") . ': ' . $this->sma->formatMoney($payment->pos_paid) . ($payment->return_id ? ' (' . lang('returned') . ')' : '').lang("no") . ': ' . 'xxxx xxxx xxxx ' . substr($payment->cc_no, -4). lang("name") . ': ' . $payment->cc_holder . PHP_EOL;   
					}
	...
	// write_file('C:/etr.txt',$data1.$data2.$data3.$data4.$data5.$data6.$data7.$data8.$data9.$data10.$data11.$data12.$data13);
